...
func _setup_teleport():
	...
	#Plugin modification
	#Set a small depth for the portal
	box.size.z = 0.1
	collider.shape = box
...
func _ready() -> void:
	...
	if portal_mesh == null:
		_setup_mesh()
	if is_teleport and teleport_area == null:
		_setup_teleport()
...
func _process_teleports() -> void:
	for body_id: int in _watchlist_teleportables.keys():
		...
		if should_teleport and abs(current_fw_angle) < teleport_tolerance:
			...
			#Plugin modification
			#Push items lying in portal boundaries apart
			if body is RigidBody3D and body.linear_velocity.length() < 0.2:
				#Find the portal's forward vector
				var portal_front = self.global_transform.basis.z.normalized()
				
				#Push the item from the portal plane based on which side it is currently leaning towards
				var push_force = 0.5
				var push_dir = portal_front * signf(current_fw_angle) 
...
func _setup_cameras() -> void:
	...
	if exit_portal != null:
		...
		#Plugin modification
		#Hide local player clones from the portal view
		portal_camera.set_cull_mask_value(19, false)
		#Hide the portal blockers from the portal view
		portal_camera.set_cull_mask_value(18, false)
		#Hide remote partner wings from the portal view
		portal_camera.set_cull_mask_value(17, false)
...
func _construct_tp_metadata(node: Node3D) -> void:
	...
	if _check_tp_interaction(TeleportInteractions.DUPLICATE_MESHES)\
		...
		for m: MeshInstance3D in meta.meshes:
			...
			#Plugin modification
			#Move local player clones to the hidden layer
			if meta.is_player:
				dupe.set_layer_mask_value(2, false)
				dupe.set_layer_mask_value(19, true)
